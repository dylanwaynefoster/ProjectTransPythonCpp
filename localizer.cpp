...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	//Trying to follow my python code here...
	int height = grid.size();
  int width = grid[0].size();
	int area = height * width;
	float belief_per_cell = 1.0 / area;
	//Run the nested for loops to fill in new grid of beliefs..
	...
	float p_miss)
{
	...
	//Initializing some of my own variables to slowly iterate through this...
	float normalizer = 0;
  int x = grid.size();
  int y = grid[0].size();
	...
	for (int i = 0; i < x; i++) {
				//Initializing row to append to grid...
				vector<float> row;
        for (int j = 0; j < y; j++) {
						if(grid[i][j] == color){
  ...
  //Loop through the grid...
	for (int i = 0; i < height; i++) {
        ...
        for (int j = 0; j < width; j++) {
					...
					newGrid[new_j][new_i] += (cell);
					// cout << new_j << "and" << new_i << endl;
					// cout << cell << endl;
